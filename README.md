...
- Pixel-perfect present scaling: nearest-neighbor at every ratio, so pixel edges stay as hard as possible at any window size (stroke widths may alternate by 1px at non-integer ratios). The letterboxed content rect is quantized to whole multiples of the game's aspect unit (640x360 -> 16x9), guaranteeing exactly square pixels at any window size. Overrides via `BUTTERSCOTCH_SCALE_FILTER`: `=pixelart` enables a single-pass pixel-art shader (t3ssel8r/SDL-style UV remap; uniform stroke widths with one-device-pixel boundary blends, slightly softer edges), `=sharp` for sharp-bilinear, `=linear` for plain bilinear, `=nearest` is the default.
